...
pumping_time = 0 # time allowed for vacuum pump to draw sample air
flow_stabilization_time = 2 # time allowed for air to settle after vacuum pump is shut off
sensing_delay_time = 19 # time delay after beginning data acquisition till when the sensor is exposed to sample
sensing_retract_time = 140 # time allowed before sensor is retracted, no longer exposed to sample
duration_of_signal = 260 # time allowed for data acquisition per test run
...
##               combinedVector, fmt='%.10f', delimiter=',')
    ...
    np.savetxt(r'/home/pi/Documents/Tests/' + str(year) + '/' + str(month) + '/' + str(day) + '/' + str(fileName) + 'MCQYFF_POS30' + '.csv',
               combinedVector, fmt='%.10f', delimiter=',')
